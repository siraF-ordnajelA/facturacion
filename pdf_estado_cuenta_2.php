...
if ($id_cliente > 0) {
   $respuesta = mysqli_query($conexion, "call select_estado_clientes($id_cliente, 2);") or die(mysqli_error());
   ...
   $pdf->Cell($width_cell[7],8,'PAGADO', 1, 0, 'C', 0);
   ...
   while ($fila = mysqli_fetch_array($respuesta)) {
      ...
      $pdf->Cell($width_cell[7],8,number_format($fila['pagado_caja'], 2, ",", "."), 1, 0, 'C', 0);
      if ($tmp_nro_comprobante != $fila['nro_comprobante']) { $deuda_total = $deuda_total + $fila['saldo_cliente']; }
      $subtotal_cobrado = $subtotal_cobrado + $fila['pagado_caja'];
      $pdf->Cell($width_cell[8],8,number_format($fila['saldo_cliente'], 2, ",", "."), 1, 1, 'C', 0);
   ...
   $respuesta = mysqli_query($conexion, "call busca_fecha_caja('$fecha1', '$fecha2', 2);") or die(mysqli_error());
