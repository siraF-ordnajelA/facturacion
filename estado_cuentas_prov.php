<?php
   ...
   $respuesta = mysqli_query($conexion, "call select_clientes(2)") or die(mysqli_error());
   ...
   if(isset($_POST["cbo_clientes"]) and !isset($_POST["btn_busca"])){
      ...
      $respuesta2 = mysqli_query($conexion2, "call select_estado_clientes($id_cliente, 2);") or die(mysqli_error());
   ...
   if(isset($_POST["btn_busca"])){
      $fecha1 = $_POST['fecha1'];
      $fecha2 = $_POST['fecha2'];
      $respuesta3 = mysqli_query($conexion3, "call busca_fecha_caja('$fecha1', '$fecha2', 2);") or die(mysqli_error());
   ...
   <form id="form_caja" method="post" action="">
      <?php 
         if ($total > 0) {
            ...
            echo '<label for="cbo_clientes">Seleccione proveedor a visualizar:&nbsp;&nbsp;</label>';
         ...
         if ($total2 > 0) {
            ...
            while ($fila2 = mysqli_fetch_array($respuesta2)) {
               if ($sw_color == 0) {
                  ...
                  echo '<td align="center" class="propio1">' .$fila2['pto_venta']. '</td>';
                  ...
                  echo '<td align="center" class="propio1">' .number_format($fila2['pagado_caja'], 2, ",", "."). '</td>';
                  if ($fila2['saldo_cliente'] < 0) { echo '<td align="center" style="background-color:rgba(41, 217, 17, 0.6);">' .number_format($fila2['saldo_cliente'], 2, ",", "."). '</td>';}
                  if ($fila2['saldo_cliente'] == 0) { echo '<td align="center" class="propio1">' .number_format($fila2['saldo_cliente'], 2, ",", "."). '</td>';}
                  if ($fila2['saldo_cliente'] > 0) { echo '<td align="center" style="background-color:rgba(224, 0, 0, 0.6);">' .number_format($fila2['saldo_cliente'], 2, ",", "."). '</td>';}
                  echo '</tr>';
                  if ($tmp_nro_comprobante != $fila2['nro_comprobante']) {
                     ...
                     $deuda_total = $deuda_total + $fila2['saldo_cliente'];
                  }
                  $total_cobrado = $total_cobrado + $fila2['pagado_caja'];
                  $sw_color = 1;
               }
               else {
                  echo '<tr>';
                  ...
                  echo '<td align="center" class="propio2">' .$fila2['pto_venta']. '</td>';
                  ...
                  echo '<td align="center" class="propio2">' .number_format($fila2['pagado_caja'], 2, ",", "."). '</td>';
                  if ($fila2['saldo_cliente'] < 0) { echo '<td align="center" style="background-color:rgba(41, 217, 17, 0.6);">' .number_format($fila2['saldo_cliente'], 2, ",", "."). '</td>';}
                  if ($fila2['saldo_cliente'] == 0) { echo '<td align="center" class="propio2">' .number_format($fila2['saldo_cliente'], 2, ",", "."). '</td>';}
                  if ($fila2['saldo_cliente'] > 0) { echo '<td align="center" style="background-color:rgba(224, 0, 0, 0.6);">' .number_format($fila2['saldo_cliente'], 2, ",", "."). '</td>';}
                  echo '</tr>';
                  if ($tmp_nro_comprobante != $fila2['nro_comprobante']) {
                     $total_importe = $total_importe + $fila2['importe'];
            ...
            if ($total2 > 9) {
               echo '<tr><td colspan="6">&nbsp;</td><td align="center" style="background-color:rgba(230, 52, 74, 0.8);">IMPORTE</td><td align="center" style="background-color:rgba(230, 52, 74, 0.8);">IVA</td><td align="center" style="background-color:rgba(230, 52, 74, 0.8);">MONTO FACTURADO</td><td align="center" style="background-color:rgba(230, 52, 74, 0.8);">PAGADO</td></tr>';
            }
            echo '</tr>';
            echo '<td class="propio2" align="right"><b>SUBTOTALES</td></b><td colspan="5" class="propio2">&nbsp;</td><td class="propio2" align="center"><b>$' . number_format($total_importe, 2, ",", ".") . '</b></td><td class="propio2" align="center"><b>$' . number_format($total_iva, 2, ",", ".") . '</b></td><td class="propio2" align="center"><b>$' . number_format($total_facturado, 2, ",", ".") . '</b></td><td class="propio2" align="center"><b>$' . number_format($total_cobrado, 2, ",", ".") . '</b></td>';
            echo '</tr>';
            ...
            //echo '<div class="titulo"><b>DEUDA:&nbsp; $'. number_format($saldo_final, 2, ",", ".") .'</b></div>';
            echo '<br><br>';
            //Paso la rezón e Id de cliente y exporto PDF
            echo '<a class="boton" href="pdf_estado_cuenta_2.php?razon=' . $razon . '&id=' . $id_cliente . '&f1=0&f2=0 " target="_blank">Exportar a PDF</a>';
            echo '<br>';
         ...
         if ($total3 > 0) {
            ...
            while ($fila3 = mysqli_fetch_array($respuesta3)) {
               if ($sw_color == 0) {
                  ...
                  echo '<td align="center" class="propio1">' .$fila3['pto_venta']. '</td>';
                  ...
                  echo '<td align="center" class="propio1">' .number_format($fila3['pagado_caja'], 2, ",", "."). '</td>';
                  if ($fila3['saldo_cliente'] < 0) { echo '<td align="center" style="background-color:rgba(41, 217, 17, 0.6);">' .number_format($fila3['saldo_cliente'], 2, ",", "."). '</td>';}
                  if ($fila3['saldo_cliente'] == 0) { echo '<td align="center" class="propio1">' .number_format($fila3['saldo_cliente'], 2, ",", "."). '</td>';}
                  if ($fila3['saldo_cliente'] > 0) { echo '<td align="center" style="background-color:rgba(224, 0, 0, 0.6);">' .number_format($fila3['saldo_cliente'], 2, ",", "."). '</td>';}
                  echo '</tr>';
                  if ($tmp_nro_comprobante != $fila3['nro_comprobante']) {
                     ...
                     $deuda_total = $deuda_total + $fila3['saldo_cliente'];
                  }
                  $total_cobrado = $total_cobrado + $fila3['pagado_caja'];
                  $sw_color = 1;
               }
               else {
                  echo '<tr>';
                  echo '<td align="center" class="propio2">' .$fila3['razon']. '</td>';
                  echo '<td align="center" class="propio2">' .$fila3['pto_venta']. '</td>';
                  ...
                  echo '<td align="center" class="propio2">' .number_format($fila3['pagado_caja'], 2, ",", "."). '</td>';
                  if ($fila3['saldo_cliente'] < 0) { echo '<td align="center" style="background-color:rgba(41, 217, 17, 0.6);">' .number_format($fila3['saldo_cliente'], 2, ",", "."). '</td>';}
                  if ($fila3['saldo_cliente'] == 0) { echo '<td align="center" class="propio2">' .number_format($fila3['saldo_cliente'], 2, ",", "."). '</td>';}
                  if ($fila3['saldo_cliente'] > 0) { echo '<td align="center" style="background-color:rgba(224, 0, 0, 0.6);">' .number_format($fila3['saldo_cliente'], 2, ",", "."). '</td>';}
                  echo '</tr>';
                  if ($tmp_nro_comprobante != $fila3['nro_comprobante']) {
                     $total_importe = $total_importe + $fila3['importe'];
            ...
            //echo '<div class="titulo"><b>SALDO:&nbsp; $'. number_format($saldo_final, 2, ",", ".") .'</b></div>';
            echo '<br><br>';
            //Paso los datos de las fechas y exporto PDF
            echo '<a class="boton" href="pdf_estado_cuenta_2.php?id=0&razon=0&f1=' . $fecha1 . '&f2=' . $fecha2 . ' " target="_blank">Exportar a PDF</a>';
            echo '<br>';
